...
public class Email {
  ...
  public static void setUsername(String user) {
    ...
    if (!user.contains("@")) {
      ...
      if (extraInfo == null) {
        System.out.println("ERROR: Note enough information for email.");
        username = null;
        props.put("mail.smtp.user", username);
        return;
      }
...
//      System.exit(0);
  ...
  public static void setCellList(String parseList) {
    ...
    try {
      ...
      emailList = null;
      System.out.println("There was a problem reading the email address list specified. Please restart the program and enter a correct list.\nList provided: " + parseList);
    }
    textEmail = null;
  }
  ...
  public static boolean sendMessage(String subject, String msg) {
    ...
    try {
      ...
      mex.printStackTrace();
      System.out.println("The message was unable to be sent.");
      return false;
    } catch (Exception e) {
      System.out.println("An unknown error has occurred while attempting to send the message.");
      e.printStackTrace();
      return false;
    }//end catch block
    return true;
